...
lin_disg = np.array([np.load("baselines/logs/disag_inverted_pendulum/dim_0_seed_{i}.npy".format(i=i)) for i in range(5)])
...
lcd = lin_disg > 0
lcd = (lcd/lin_disg.shape[-1]).sum(axis=-1)[:, :-39]
...
plt.savefig("plots/x_cov.pdf", bbox_inches='tight')
...
ang_disg = np.array([np.load("baselines/logs/disag_inverted_pendulum/dim_1_seed_{i}.npy".format(i=i)) for i in range(5)])
...
acd = ang_disg > 0
acd = (acd/ang_disg.shape[-1]).sum(axis=-1)[:, :-39]
...
plt.plot(x, np.ones(len(x)), color=green,)
plt.legend()
plt.title("$\\theta$ Coverage")
plt.xlabel("Num Env. Steps")
plt.savefig("plots/theta_cov.pdf", bbox_inches='tight')
